...
export function statusChip(status, t = (k) => k) {
  ...
  if (s === 'ARCHIVED') return <span className="chip navy"><span className="chip-dot"></span>{t('status_archived')}</span>;
  return <span className="chip"><span className="chip-dot"></span>{status || '—'}</span>;
}
